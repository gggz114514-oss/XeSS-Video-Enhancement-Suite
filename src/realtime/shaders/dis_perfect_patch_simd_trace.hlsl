...
RWTexture2D<float2> Trace : register(u1);

// rung-2 trace layout inside the L2 dense (48 rows x 64 cols, R32G32):
//   rows 0..31   : per-lane per-row addends (lane*8 + r):
//                  cell0=(d_l, d_r)  cell1=(sd_before, sq_before)
//                  cell2=(t_sd, t_sq) cell3=(sd_after, sq_after)
//   rows 32..35  : lane finals (sd_lane, sq_lane)
//   row 36       : ((v0+v2), (v1+v3)) of the sd tree
//   row 37       : ((v0+v2), (v1+v3)) of the sq tree
//   row 38       : (s_diff, s_sq)
//   row 39       : (ssd, n)
...
void patch_residual_trace(uint px, uint py, float ux, float uy, uint lane, bool trace,
                          out float out_sd, out float out_sq)
{
    precise float i_I1 = min(max((float)py + uy + 16.0, 9.0), (float)height + 15.0);
    precise float j_I1 = min(max((float)px + ux + 16.0, 9.0), (float)width + 15.0);
    ...
    for (uint r = 0; r < 8; ++r)
    {
        ...
        {
            uint row = lane * 8 + r;
            Trace[uint2(0, row)] = float2(d_l, d_r);
            Trace[uint2(1, row)] = float2(sd, sq);
            Trace[uint2(2, row)] = float2(t_sd, t_sq);
            Trace[uint2(3, row)] = float2(sd + t_sd, sq + t_sq);
        }
        sd = sd + t_sd;
        sq = sq + t_sq;
    }
...
float candidate_ssd_trace(uint px, uint py, float2 u, uint patch, uint lane)
{
    float sd, sq;
    patch_residual_trace(px, py, u.x, u.y, lane, patch == 0, sd, sq);
    if (lane < 4 && patch == 0)
        Trace[uint2(0, 32 + lane)] = float2(sd, sq);
    reduce_lanes(patch, lane, sd);
    float s_diff = gShare[patch][2];
    GroupMemoryBarrierWithGroupSync();
    if (lane == 0 && patch == 0)
        Trace[uint2(0, 36)] = float2(gShare[0][0], gShare[0][1]);
    GroupMemoryBarrierWithGroupSync();
    reduce_lanes(patch, lane, sq);
    float s_sq = gShare[patch][2];
    if (lane == 0 && patch == 0)
        Trace[uint2(0, 37)] = float2(gShare[0][0], gShare[0][1]);
    float n = 64.0;
    precise float ssd = s_sq - s_diff * s_diff / n;
    if (lane == 0 && patch == 0)
    {
        Trace[uint2(0, 38)] = float2(s_diff, s_sq);
        Trace[uint2(1, 38)] = float2(ssd, n);
    }
    return ssd;
}
                  ...
                  out float2 out_flow, out bool out_ok)
{
    ...
    for (uint t = 0; t < gdIterations; ++t)
    {
        ...
        if (patch == 0 && lane == 0)
        {
            Trace[uint2(0, 40 + t)] = float2(ssd, 0.0);
            Trace[uint2(1, 40 + t)] = float2(cur.x, cur.y);
            Trace[uint2(2, 40 + t)] = float2(dUx, dUy);
            Trace[uint2(3, 40 + t)] = float2(dx, dy);
            Trace[uint2(4, 40 + t)] = float2(inv11, inv12);
            Trace[uint2(5, 40 + t)] = float2(inv22, 0.0);
        }
...
{
    if (groupThread.x == 0)
        Trace[uint2(6, 47)] = float2(12345.0, 6789.0);
    ...
    uint2 coord = traceExplicit != 0 ? uint2(traceSj, traceSi) : patch_coordinate(ordinal - first);
    ...
    float best = candidate_ssd_trace(px, py, seed, patch, lane);
    float2 best_flow = seed;
    float seed_sd_t = best, side_sd_t = 1e30, perp_sd_t = 1e30;
    float2 side_u_t = float2(0, 0), perp_u_t = float2(0, 0);
    GroupMemoryBarrierWithGroupSync();
    if (lane == 0 && patch == 0)
        Trace[uint2(0, 60)] = float2(seed.x, seed.y);
    if (direction == 0)
    {
        if (sj > 0)
        {
    ...
    if (lane == 0 && patch == 0)
    {
        Trace[uint2(0, 61)] = float2(seed.x, seed.y);
        Trace[uint2(1, 61)] = float2(best, best_flow.x);
        Trace[uint2(2, 61)] = float2(best_flow.y, seed_sd_t);
    }
    float2 flow;
    bool ok;
    GroupMemoryBarrierWithGroupSync();
    ...
    GroupMemoryBarrierWithGroupSync();
    if (lane == 0 && patch == 0)
        Trace[uint2(0, 62)] = float2(flow.x, flow.y);
    if (lane == 0 && patch == 0 && direction != 0)
    {
        Trace[uint2(2, 39)] = float2(seed_sd_t, 0.0);
        Trace[uint2(3, 39)] = float2(side_sd_t, side_u_t.x);
        Trace[uint2(4, 39)] = float2(side_u_t.y, perp_sd_t);
        Trace[uint2(5, 39)] = float2(perp_u_t.x, perp_u_t.y);
    }
    float2 result = ok ? flow : seed;
    if (lane == 0 && traceNoWrite == 0)
