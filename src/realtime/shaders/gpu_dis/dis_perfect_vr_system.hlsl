...
float forward_weight(float2 work, float2 right_work, float2 down_work)
{
    ...
    return (alpha * 0.5) / sqrt(ux * ux + vx * vx + uy * uy + vy * vy + epsilon * epsilon);
}
...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    float zeta2 = 0.1 * 0.1, eps2 = epsilon * epsilon;
    float deriv = ix * ix + iy * iy + zeta2;
    float residual = iz + ix * dw.x + iy * dw.y;
    float w = (delta * 0.5) / sqrt(residual * residual / deriv + eps2) / deriv;
    ...
    float gw = (gamma * 0.5) / sqrt(qx * qx / n1 + qy * qy / n2 + eps2);
    a11 += gw * (ixx * ixx / n1 + ixy * ixy / n2);
    a12 += gw * (ixx * ixy / n1 + ixy * iyy / n2);
    a22 += gw * (ixy * ixy / n1 + iyy * iyy / n2);
    b1 -= gw * (ixx * ixz / n1 + ixy * iyz / n2);
    b2 -= gw * (ixy * ixz / n1 + iyy * iyz / n2);
    ...
    if (p.x + 1 < (int)width) { edge = wcur; a11 += edge; a22 += edge; float2 d = edge * (flow_at(BaseFlow,p+int2(1,0)) - base); b1 += d.x; b2 += d.y; }
    if (p.x > 0)
    {
        int2 left_p = p - int2(1, 0);
        edge = forward_weight(flow_at(WorkFlow, left_p), work, flow_at(WorkFlow, left_p + int2(0, 1)));
        a11 += edge; a22 += edge; float2 d = edge * (base - flow_at(BaseFlow,left_p)); b1 -= d.x; b2 -= d.y;
    }
    if (p.y + 1 < (int)height) { edge = wcur; a11 += edge; a22 += edge; float2 d = edge * (flow_at(BaseFlow,p+int2(0,1)) - base); b1 += d.x; b2 += d.y; }
    if (p.y > 0)
    {
        int2 up = p - int2(0, 1);
        edge = forward_weight(flow_at(WorkFlow, up), flow_at(WorkFlow, up + int2(1, 0)), work);
        a11 += edge; a22 += edge; float2 d = edge * (base - flow_at(BaseFlow,up)); b1 -= d.x; b2 -= d.y;
    }
