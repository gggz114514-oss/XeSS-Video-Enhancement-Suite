...
bool parse_args(int argc, char** argv, Args& args) {
    for (int i = 1; i < argc; ++i) {
        ...
        else if (!strcmp(key, "--gpu-block-motion")) args.gpu_block_motion = true;
...
bool init_gpu_block_motion(const Args& args, Runtime& runtime) {
    runtime.gpu_block_motion = args.gpu_block_motion;
    if (!runtime.gpu_block_motion) return true;
    runtime.motion_repair_mode = args.motion_repair == "propagate" ? 1u :
                                 args.motion_repair == "refine" ? 2u : 0u;
    runtime.motion_center_bias = args.motion_center_bias;

    D3D12_DESCRIPTOR_HEAP_DESC heap_desc{};
    heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    // 0..39 are the existing motion tables.  The library full-GPU path uses
    // 40..46 for the per-frame NV12 planes and RGBA/luma UAVs, and 47 for
    // the constant-depth UAV.
    heap_desc.NumDescriptors = 48 * std::max<uint32_t>(1, args.motion_descriptor_slots);
    heap_desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    if (!hr_ok(runtime.device->CreateDescriptorHeap(
                   &heap_desc, IID_PPV_ARGS(&runtime.motion_heap)),
               "CreateDescriptorHeap(gpu-block)")) return false;
    runtime.motion_descriptor_stride = runtime.device->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    if (!create_motion_root(runtime.device.Get(), runtime.motion_root)) return false;
    const std::string shader_dir = args.shader_dir;
    if (shader_dir.empty()) {
        fprintf(stderr, "[gpu-block] --shader-dir is required\n");
        return false;
    }
    if (!load_motion_pipeline(runtime.device.Get(), shader_dir,
                              "surface_rgba_luma", runtime.motion_root.Get(),
                              runtime.rgba_luma_pso) ||
        !load_motion_pipeline(runtime.device.Get(), shader_dir,
                              "surface_gpu_motion_tile", runtime.motion_root.Get(),
                              runtime.block_motion_pso) ||
        !load_motion_pipeline(runtime.device.Get(), shader_dir,
                              "surface_gpu_motion_repair", runtime.motion_root.Get(),
                              runtime.repair_pso) ||
        !load_motion_pipeline(runtime.device.Get(), shader_dir,
                              "surface_gpu_velocity", runtime.motion_root.Get(),
                              runtime.velocity_pso) ||
        (args.full_gpu && !load_motion_pipeline(
            runtime.device.Get(), shader_dir, "surface_nv12_color",
            runtime.motion_root.Get(), runtime.nv12_color_pso))) return false;

    const D3D12_RESOURCE_FLAGS uav = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    for (int i = 0; i < 2; ++i) {
        runtime.luma[i] = create_texture(
            runtime.device.Get(), args.width, args.height, DXGI_FORMAT_R32_FLOAT,
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            i ? L"XeSS-FG GPU Block luma 1" : L"XeSS-FG GPU Block luma 0", uav);
    }
    runtime.forward_flow = create_texture(
        runtime.device.Get(), args.width, args.height, DXGI_FORMAT_R16G16_FLOAT,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"XeSS-FG GPU Block forward", uav);
    runtime.backward_flow = create_texture(
        runtime.device.Get(), args.width, args.height, DXGI_FORMAT_R16G16_FLOAT,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"XeSS-FG GPU Block backward", uav);
    runtime.motion_confidence = create_texture(
        runtime.device.Get(), args.width, args.height, DXGI_FORMAT_R32_FLOAT,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"XeSS-FG GPU Block confidence", uav);
    runtime.repaired_flow = create_texture(
        runtime.device.Get(), args.width, args.height, DXGI_FORMAT_R16G16_FLOAT,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"XeSS-FG GPU Block repaired", uav);
    if (args.full_gpu) {
        runtime.decoded_color = create_texture(
            runtime.device.Get(), args.width, args.height,
            DXGI_FORMAT_R8G8B8A8_UNORM,
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            L"XeSS-FG decoded RGBA", uav);
    }
    if (!runtime.luma[0] || !runtime.luma[1] || !runtime.forward_flow ||
        !runtime.backward_flow || !runtime.motion_confidence ||
        !runtime.repaired_flow || (args.full_gpu && !runtime.decoded_color))
        return false;
    fprintf(stderr,
            "[gpu-block] motion source ready: current->previous, repair=%s, center-bias=%.6f\n",
            args.motion_repair.c_str(), args.motion_center_bias);
    return true;
}

void write_motion_pass_descriptors(
        Runtime& runtime, UINT srv_base, UINT uav_base,
        ID3D12Resource* t0, DXGI_FORMAT t0_format,
        ID3D12Resource* t1, DXGI_FORMAT t1_format,
        ID3D12Resource* t2, DXGI_FORMAT t2_format,
        ID3D12Resource* t3, DXGI_FORMAT t3_format,
        ID3D12Resource* t4, DXGI_FORMAT t4_format,
        ID3D12Resource* u0, DXGI_FORMAT u0_format,
        ID3D12Resource* u1, DXGI_FORMAT u1_format) {
    ID3D12Device* device = runtime.device.Get();
    ID3D12DescriptorHeap* heap = runtime.motion_heap.Get();
    const UINT stride = runtime.motion_descriptor_stride;
    create_srv(device, heap, srv_base + 0, stride, t0, t0_format);
    create_srv(device, heap, srv_base + 1, stride, t1, t1_format);
    create_srv(device, heap, srv_base + 2, stride, t2, t2_format);
    create_srv(device, heap, srv_base + 3, stride, t3, t3_format);
    create_srv(device, heap, srv_base + 4, stride, t4, t4_format);
    create_uav(device, heap, uav_base + 0, stride, u0, u0_format);
    create_uav(device, heap, uav_base + 1, stride, u1, u1_format);
}

bool record_gpu_block_motion(Runtime& runtime, const Args& args,
                             ID3D12Resource* backbuffer, int frame_index,
                             bool luma_precomputed = false) {
    ID3D12GraphicsCommandList* list = runtime.list.Get();
    const UINT current = static_cast<UINT>(frame_index) & 1u;
    const UINT previous = current ^ 1u;
    if (!luma_precomputed &&
        runtime.luma_state[current] != D3D12_RESOURCE_STATE_UNORDERED_ACCESS) {
        transition(list, runtime.luma[current].Get(), runtime.luma_state[current],
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        runtime.luma_state[current] = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    }
    if (frame_index > 1 && runtime.motion_outputs_are_srv) {
        transition(list, runtime.forward_flow.Get(),
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        transition(list, runtime.backward_flow.Get(),
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        transition(list, runtime.motion_confidence.Get(),
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        if (runtime.motion_repair_mode != 0u)
            transition(list, runtime.repaired_flow.Get(),
                       D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                       D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    }
    if (frame_index > 0) {
        transition(list, runtime.velocity.Get(),
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    }
    ...
    auto bind = [&](UINT srv_base, UINT uav_base) {
        list->SetComputeRootDescriptorTable(
            0, gpu_descriptor(runtime.motion_heap.Get(), srv_base,
                              runtime.motion_descriptor_stride));
        list->SetComputeRootDescriptorTable(
            1, gpu_descriptor(runtime.motion_heap.Get(), uav_base,
                              runtime.motion_descriptor_stride));
    };
    UINT constants[6] = {static_cast<UINT>(args.width),
                         static_cast<UINT>(args.height),
                         static_cast<UINT>(args.width),
                         static_cast<UINT>(args.height), 0u, 0u};
    auto set_constants = [&]() {
        list->SetComputeRoot32BitConstants(2, 6, constants, 0);
    };
    if (!luma_precomputed) {
        // Each dispatch owns a distinct descriptor range: descriptors are
        // read at GPU execution time, so later recording must not overwrite
        // the table used by this dispatch.
        write_motion_pass_descriptors(
            runtime, 0, 5,
            ...
            backbuffer, DXGI_FORMAT_R8G8B8A8_UNORM,
            backbuffer, DXGI_FORMAT_R8G8B8A8_UNORM,
            runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
            runtime.motion_confidence.Get(), DXGI_FORMAT_R32_FLOAT);
        list->SetPipelineState(runtime.rgba_luma_pso.Get());
        bind(0, 5);
        set_constants();
        list->Dispatch((args.width + 7) / 8, (args.height + 7) / 8, 1);
        transition(list, runtime.luma[current].Get(),
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        runtime.luma_state[current] =
            D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
    } else if (runtime.luma_state[current] !=
               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE) {
        fprintf(stderr, "[gpu-block] precomputed luma has invalid state\n");
        return false;
    }

    if (frame_index == 0) {
        // First present establishes history; no inter-frame motion exists.
        write_motion_pass_descriptors(
            runtime, 32, 37,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.velocity.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.motion_confidence.Get(), DXGI_FORMAT_R32_FLOAT);
        const float zero[4] = {0, 0, 0, 0};
        list->ClearUnorderedAccessViewFloat(
            gpu_descriptor(runtime.motion_heap.Get(), 37,
                           runtime.motion_descriptor_stride),
            cpu_descriptor(runtime.motion_heap.Get(), 37,
                           runtime.motion_descriptor_stride),
            runtime.velocity.Get(), zero, 0, nullptr);
        transition(list, runtime.velocity.Get(),
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        transition(list, backbuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                   D3D12_RESOURCE_STATE_PRESENT);
        return true;
    }

    // Current->previous (XeFG direction), then previous->current for the
    // confidence/repair consistency test.
    write_motion_pass_descriptors(
        runtime, 8, 13,
        runtime.luma[previous].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
    ...
    write_motion_pass_descriptors(
        ...
        runtime.luma[previous].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.luma[previous].Get(), DXGI_FORMAT_R32_FLOAT,
        runtime.backward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
        runtime.motion_confidence.Get(), DXGI_FORMAT_R32_FLOAT);
    list->SetPipelineState(runtime.block_motion_pso.Get());
    bind(8, 13);
    constants[4] = 1u;
    std::memcpy(&constants[5], &runtime.motion_center_bias, sizeof(float));
    set_constants();
    list->Dispatch((args.width + 7) / 8, (args.height + 7) / 8, 1);
    bind(16, 21);
    constants[4] = 0u;
    ...
    ID3D12Resource* selected_flow = runtime.forward_flow.Get();
    if (runtime.motion_repair_mode != 0u) {
        write_motion_pass_descriptors(
            runtime, 24, 29,
            runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            ...
            runtime.luma[current].Get(), DXGI_FORMAT_R32_FLOAT,
            runtime.luma[previous].Get(), DXGI_FORMAT_R32_FLOAT,
            runtime.repaired_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
            runtime.motion_confidence.Get(), DXGI_FORMAT_R32_FLOAT);
        list->SetPipelineState(runtime.repair_pso.Get());
        bind(24, 29);
        constants[4] = runtime.motion_repair_mode;
        set_constants();
        list->Dispatch((args.width + 7) / 8, (args.height + 7) / 8, 1);
        transition(list, runtime.repaired_flow.Get(),
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        selected_flow = runtime.repaired_flow.Get();
    }

    write_motion_pass_descriptors(
        runtime, 32, 37,
        selected_flow, DXGI_FORMAT_R16G16_FLOAT,
        runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
        runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
        runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
        runtime.forward_flow.Get(), DXGI_FORMAT_R16G16_FLOAT,
    ...
    bind(32, 37);
    constants[0] = static_cast<UINT>(args.width);
    constants[1] = static_cast<UINT>(args.height);
    constants[2] = static_cast<UINT>(args.width);
    constants[3] = static_cast<UINT>(args.height);
    constants[4] = 0u;
    constants[5] = 0u;
    set_constants();
    list->Dispatch((args.width + 7) / 8, (args.height + 7) / 8, 1);
    ...
    transition(list, backbuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
               D3D12_RESOURCE_STATE_PRESENT);
    runtime.motion_outputs_are_srv = true;
    return true;
}
...
bool init_runtime(const Args& args, Runtime& runtime) {
    ...
    if (!init_gpu_block_motion(args, runtime)) return false;
    if (args.full_gpu) {
...
bool upload_frame(Runtime& runtime, const Args& args, const uint8_t* rgb, const float* motion,
    ...
    if (args.gpu_block_motion) {
        transition(runtime.list.Get(), backbuffer, D3D12_RESOURCE_STATE_COPY_DEST,
                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        if (!record_gpu_block_motion(runtime, args, backbuffer, frame_index)) return false;
    } else {
        transition(runtime.list.Get(), backbuffer, D3D12_RESOURCE_STATE_COPY_DEST,
                   D3D12_RESOURCE_STATE_PRESENT);
